...
```{r}
enc <- read.csv("2025/data/UEF_AST_SNS_SE.csv")
glimpse(enc)
```

## Separate Acoustic and Pit

```{r}
enc_ac <- enc %>% 
  filter(TagType == "Acoustic")

enc_PIT <- enc %>% 
  filter(TagType == "PIT")

### confirm dim rows numbers with tagging records
dim(enc_ac)
dim(enc_PIT)
```
...
str(acvalues)
acvalues$TagSerialNumber
acvalues$Acoustic_ID
